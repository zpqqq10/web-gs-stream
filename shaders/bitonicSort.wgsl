...
fn main(
  @builtin(global_invocation_id) global_id: vec3<u32>, // [0,1,2,3, ..., WORKGROUP.xyz]
    ...
    for (var i = startIdx; i < endIdx; i++ ) {
        ...
        let swap0 = (i & k) == 0 && depthBuffer[i] > depthBuffer[i_XOR_j];
        let swap1 = (i & k) != 0 && depthBuffer[i] < depthBuffer[i_XOR_j];
        if (swap0 || swap1) {
            // WIKIPEDIA: swap the elements arr[i] and arr[i_XOR_j]
            let tmp0 = depthBuffer[i];
            depthBuffer[i] = depthBuffer[i_XOR_j];
            depthBuffer[i_XOR_j] = tmp0;
            
            let tmp1 = depthIndex[i];
            depthIndex[i] = depthIndex[i_XOR_j];
            depthIndex[i_XOR_j] = tmp1;
        }
    }
